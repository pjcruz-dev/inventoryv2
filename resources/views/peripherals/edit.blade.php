        ...
        <nav class="flex mb-5" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <span class="ml-1 text-gray-500 md:ml-2">Edit {{ $peripheral->asset->asset_tag }}</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex flex-wrap items-center justify-between mb-6">
            <div class="flex flex-col">
                <h1 class="mb-0 font-bold text-transparent text-3xl bg-gradient-to-tl from-orange-500 to-yellow-500 bg-clip-text">
                    <i class="fas fa-edit mr-3"></i>Edit Peripheral
                </h1>
                <p class="mb-0 text-sm leading-normal text-gray-500">Update peripheral information for {{ $peripheral->asset->asset_tag }}</p>
            </div>
            <div class="flex flex-wrap items-center space-x-2">
                <a href="{{ route('peripherals.show', $peripheral) }}" class="inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle transition-all rounded-lg cursor-pointer bg-gradient-to-tl from-blue-600 to-cyan-400 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs">
            ...
            <div class="w-full max-w-full px-3">
                <div class="relative flex flex-col min-w-0 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                    <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                        ...
                        <p class="text-sm leading-normal text-slate-400">Update the details for this peripheral</p>
                    </div>
                    <div class="flex-auto px-6 pt-0 pb-6">
                        <form action="{{ route('peripherals.update', $peripheral) }}" method="POST" class="mt-4">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="w-full max-w-full px-3 md:w-6/12 md:flex-none">
                                    <div class="mb-4">
                                        ...
                                        <select name="type" id="type" required class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow @error('type') border-red-500 @enderror">
                                            ...
                                            <option value="Drawing Tablet" {{ old('type', $peripheral->type) == 'Drawing Tablet' ? 'selected' : '' }}>Drawing Tablet</option>
                                            <option value="Game Controller" {{ old('type', $peripheral->type) == 'Game Controller' ? 'selected' : '' }}>Game Controller</option>
                                ...
                                <div class="w-full max-w-full px-3 md:w-6/12 md:flex-none">
                                    <div class="mb-4">
                                        ...
                                        <select name="interface" id="interface" required class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow @error('interface') border-red-500 @enderror">
                                            ...
                                            <option value="PS/2" {{ old('interface', $peripheral->interface) == 'PS/2' ? 'selected' : '' }}>PS/2</option>
                                            <option value="USB-C" {{ old('interface', $peripheral->interface) == 'USB-C' ? 'selected' : '' }}>USB-C</option>
                                            <option value="3.5mm Jack" {{ old('interface', $peripheral->interface) == '3.5mm Jack' ? 'selected' : '' }}>3.5mm Jack</option>
                                            <option value="HDMI" {{ old('interface', $peripheral->interface) == 'HDMI' ? 'selected' : '' }}>HDMI</option>
                                            <option value="Other" {{ old('interface', $peripheral->interface) == 'Other' ? 'selected' : '' }}>Other</option>
                                        </select>
                                        @error('interface')
                                            <p class="mt-2 text-xs text-red-500">{{ $message }}</p>
                                        @enderror
                            ...
                            <div class="flex flex-wrap items-center justify-end mt-6 pt-6 border-t border-gray-200">
                                <a href="{{ route('peripherals.show', $peripheral) }}" class="inline-block px-6 py-3 mr-3 font-bold text-center text-slate-500 uppercase align-middle transition-all rounded-lg cursor-pointer hover:scale-102 active:opacity-85 hover:shadow-soft-xs">
                                    Cancel
                                </a>
                                <button type="submit" class="inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle transition-all rounded-lg cursor-pointer bg-gradient-to-tl from-orange-500 to-yellow-500 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs">
                                    <i class="fas fa-save mr-2"></i>Update Peripheral
                                </button>
                            </div>
                        </form>
                    </div>
...
document.getElementById('asset_id').addEventListener('change', function() {
    ...
    // Only auto-suggest if current type is empty
    if (!typeSelect.value) {
...
document.getElementById('type').addEventListener('change', function() {
    ...
    // Only auto-suggest if current interface is empty
