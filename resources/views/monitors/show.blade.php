                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <div>
                                    @if($monitor->asset->assignedUser)
                                        <button type="button" class="btn btn-outline-danger me-2" onclick="unassignAsset({{ $monitor->asset->id }})">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="new_assigned_to" name="new_assigned_to" required size="6" style="height: 150px;">
                            <option value="">Choose a user...</option>
                            @foreach(\App\Models\User::where('status', 'active')->where('id', '!=', $monitor->asset->assigned_to)->orderBy('first_name')->get() as $user)
                                <option value="{{ $user->id }}" data-name="{{ strtolower($user->first_name . ' ' . $user->last_name) }}" data-department="{{ strtolower($user->department->name ?? '') }}">{{ $user->first_name }} {{ $user->last_name }} ({{ $user->department->name ?? 'No Department' }})</option>
                            @endforeach
                        </select>
                    </div>
...
function unassignAsset(assetId) {
    if (confirm('Are you sure you want to unassign this user from the monitor?')) {
