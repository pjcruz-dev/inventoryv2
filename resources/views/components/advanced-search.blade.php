@props(['route', 'filters' => [], 'activeFilters' => []])
...
<div class="mt-3">
    <div class="row">
        <div class="col-md-8">
            <form method="GET" action="{{ $route }}" id="searchForm">
                <div class="input-group">
                    <input type="text" name="search" class="form-control" placeholder="{{ $attributes->get('placeholder', 'Search...') }}" value="{{ request('search') }}" style="border-radius: 6px 0 0 6px; border: 2px solid #e9ecef;">
                    ...
                    <button type="button" class="btn" id="toggleAdvancedSearch" style="border-radius: 0 6px 6px 0; color: white; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border: 2px solid #667eea;">
                        <i class="fas fa-filter"></i> <span id="advancedBtnText">{{ count($activeFilters) > 0 ? 'Hide Filters' : 'Advanced' }}</span>
                    </button>
                </div>
            </form>
        </div>
        <div class="col-md-4 text-end">
            @php
                $totalFilters = count($activeFilters) + (request()->filled('search') ? 1 : 0);
            @endphp
            @if($totalFilters > 0)
                <span class="badge bg-primary me-2" style="padding: 0.5rem 0.75rem; font-size: 0.875rem;">
                    <i class="fas fa-filter me-1"></i>{{ $totalFilters }} Active Filter{{ $totalFilters > 1 ? 's' : '' }}
                </span>
    ...
    <div id="advancedSearchPanel" class="mt-3" style="display: {{ count($activeFilters) > 0 ? 'block' : 'none' }};">
                ...
                <form method="GET" action="{{ $route }}" id="advancedSearchForm">
                    ...
                    <div class="row mt-3">
                        <div class="col-12 text-end">
                            <button type="button" class="btn btn-outline-secondary" onclick="document.getElementById('advancedSearchForm').reset(); window.location.href='{{ $route }}';">
